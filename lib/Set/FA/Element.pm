...
{
	my($self, $arg)    = @_;
	$$arg{accepting}   ||= []; # Caller must set.
	$$arg{actions}     ||= {}; # Caller must set.
	$$arg{current}     = '';
...
sub reset
{
	...
	# Warning: return $self -> current($self -> start) returns $self.
...
Note: This method, called with a parameter, returns the DFA object, not the 'current' state.
...
You can't do this:
	...
	my($current) = $dfa -> current($dfa -> current($new_state) );

This is with L<Hash::FieldHash> V 0.10.
...
Provide a logger object. This object replaces the object supplied in the call to new(),
if any.
...
Resets the state machine to the start state.

Returns the 'current' state, which will be the start state.

Does not reset the id or the data associated with the object.
...
=item Compares the 'current' state to $next
...
=head2 What's different with V 1.00 of L<Set::FA::Element>?
...
=item o Rename the parameter accept to accepting
...
=item o Chop the states parameter
